...
def generateWorkflow():
	...
	if contract_pre_post.check_fix_value_range(value='United States', data_dictionary=mapping_Country__input_dataDictionary_df, belong_op=Belong(0), field='Country',
		...
		print('PRECONDITION mapping(Country)_PRE_valueRange VALIDATED')
	else:
		print('PRECONDITION mapping(Country)_PRE_valueRange NOT VALIDATED')
	
	if contract_pre_post.check_fix_value_range(value='United States', data_dictionary=mapping_Country__output_dataDictionary_df, belong_op=Belong(0), field='Country',
									quant_abs=None, quant_rel=None, quant_op=None):
		print('POSTCONDITION mapping(Country)_POST_valueRange VALIDATED')
	else:
		print('POSTCONDITION mapping(Country)_POST_valueRange NOT VALIDATED')
	
	
	input_values_list_def_INV_condition=['United States']
	output_values_list_def_INV_condition=['USA']
	...
	if contract_invariants.check_inv_fix_value_fix_value(data_dictionary_in=mapping_Country__input_dataDictionary_df,
		...
		print('INVARIANT mapping(Country)_INV_condition VALIDATED')
	else:
		print('INVARIANT mapping(Country)_INV_condition NOT VALIDATED')
